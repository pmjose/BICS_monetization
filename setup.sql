...
-- Replace <your_warehouse>, <your_role>, and file paths as needed.
------------------------------------------------------------------------
...
CREATE STAGE IF NOT EXISTS BICS_TELCO.MOBILITY_DATA.BICS_STREAMLIT_STAGE
    ...
    COMMENT        = 'Stage for Streamlit app files and CSV data loads';
...
------------------------------------------------------------------------
-- 8. UPLOAD DATA FILES TO STAGE
------------------------------------------------------------------------
-- Replace /path/to with the actual local directory containing the CSVs.

-- Mobility CSVs
PUT file:///path/to/bics_roaming_brussels_capital_jan2026.csv @BICS_TELCO.MOBILITY_DATA.BICS_STREAMLIT_STAGE/data/ AUTO_COMPRESS=TRUE OVERWRITE=TRUE;
PUT file:///path/to/bics_roaming_flanders_jan2026.csv          @BICS_TELCO.MOBILITY_DATA.BICS_STREAMLIT_STAGE/data/ AUTO_COMPRESS=TRUE OVERWRITE=TRUE;
PUT file:///path/to/bics_roaming_wallonia_jan2026.csv           @BICS_TELCO.MOBILITY_DATA.BICS_STREAMLIT_STAGE/data/ AUTO_COMPRESS=TRUE OVERWRITE=TRUE;

-- IoT CSVs
PUT file:///path/to/bics_iot_devices.csv   @BICS_TELCO.MOBILITY_DATA.BICS_STREAMLIT_STAGE/data/ AUTO_COMPRESS=TRUE OVERWRITE=TRUE;
PUT file:///path/to/bics_iot_telemetry.csv @BICS_TELCO.MOBILITY_DATA.BICS_STREAMLIT_STAGE/data/ AUTO_COMPRESS=TRUE OVERWRITE=TRUE;
...
------------------------------------------------------------------------
-- Upload app files (replace /path/to with actual paths)
...
PUT file:///path/to/streamlit_app/logo.jpg          @BICS_TELCO.MOBILITY_DATA.BICS_STREAMLIT_STAGE OVERWRITE=TRUE AUTO_COMPRESS=FALSE;
...
PUT file:///path/to/streamlit_app/utils/*.py        @BICS_TELCO.MOBILITY_DATA.BICS_STREAMLIT_STAGE/utils/  OVERWRITE=TRUE AUTO_COMPRESS=FALSE;
